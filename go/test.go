...
func main() {
	initialModel := model{0, false, 10, 0, 0, false, false, []string{"test"},}
...
func loadChoicesFromJSON(filename string) ([]string) {
...
// Update progress bar
func updateBar(msg tea.Msg, m model) (tea.Model, tea.Cmd) {
	switch msg.(type) {
	case frameMsg:
		if !m.Loaded {
			m.Frames++
...
func homePage(m model) string {
	...
	choices := fmt.Sprintf(
		"%s\n%s\n%s\n%s",
		checkbox("Verse Lookup", c == 0),
		checkbox("Continue Reading", c == 1),
		checkbox("Random Verse", c == 2),
		checkbox("Start Genesis 1:1", c == 3),
	)
...
func secondPage(m model) string {
	var msg string
...
func Genesis(m model) string {
    s := "Welcome to Genesis \n"
    m.Choice = loadChoicesFromJSON("./../bible/kjv.json")
	for choice := range m.Choice {
